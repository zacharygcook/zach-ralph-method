...
declare -A FINAL_HOOK_STATUS=(
  [review]="pending"
  [documentation]="pending"
  [tests]="pending"
)
declare -A FINAL_HOOK_REASON=(
  [review]="null"
  [documentation]="null"
  [tests]="null"
)
...
record_hook_result() {
  ...
  local status="$2"
  local reason="${3:-null}"
  FINAL_HOOK_STATUS["$hook_key"]="$status"
  FINAL_HOOK_REASON["$hook_key"]="$reason"
}

persist_hook_results() {
  local hook_key
  for hook_key in review documentation tests; do
    update_manifest_hook "$hook_key" "${FINAL_HOOK_STATUS[$hook_key]}" "${FINAL_HOOK_REASON[$hook_key]}"
  done
}
