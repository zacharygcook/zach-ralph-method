...
Vendor a new package with `npx skills@latest add zacharygcook/zach-ralph-method --skill ralph-workflows --copy`, or refresh a locked package with `npx skills@latest update ralph-workflows --project`. Then use the vendored `scripts/ralph` launcher to initialize or upgrade the stateful project runtime and run `validate` before live execution. In multi-repo mode, ensure project-supplied validation commands cover contracts that cross repository boundaries.
